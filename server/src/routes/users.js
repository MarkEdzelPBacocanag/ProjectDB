...
router.post('/register', async (req, res) => {
  try {
    ...
    if (role === 'staff') {
      ...
      if (!token) return res.status(401).json({ message: 'Admin token required to create staff account' });
      try {
    ...
    if (role === 'staff') {
      if (!staffId) return res.status(400).json({ message: 'staffId required for staff role' });
    ...
    const user = await User.create({ username, password: hashed, role, staff: staffRef });
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ token, user: { id: user._id, username: user.username, role: user.role, staff: user.staff } });
  } catch (e) {
    res.status(500).json({ message: e.message });
  }
});
